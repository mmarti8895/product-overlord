 ...
 */
export function emitCommentDraft(result, run_id, opts = {}) {
    ...
    if (shadowMode) {
        ...
        process.stdout.write(JSON.stringify({ level: "info", message: "shadow_mode_draft", context: { run_id, ticket_key: result.ticket_key }, timestamp: new Date().toISOString() }) + "\n");
    }
    const lines = [];
    // ── Header ────────────────────────────────────────────────────────────────
    ...
    lines.push("");
    // ── Verdict badge ─────────────────────────────────────────────────────────
    const verdictIcon = result.readiness_status === "ready"
        ? "✅"
        : result.readiness_status === "blocked"
            ? "🚫"
            : "⚠️";
    lines.push(`*Verdict:* ${verdictIcon} ${result.readiness_status.toUpperCase().replace("_", " ")}  ` +
    ...
    // ── Audit footer ──────────────────────────────────────────────────────────
    lines.push("----");
    lines.push(`_Generated by product-overlord · run_id: \`${run_id}\` · This comment requires human approval before posting._`);
    return {
        run_id,
