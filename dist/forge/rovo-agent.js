 ...
 */
import { handleIngestIssue, handleIngestBoard, handleConfirmPost, } from "./endpoints.js";
...
// ---------------------------------------------------------------------------
function toForgeReq(authToken, body, params, query) {
    return {
...
export async function analyseTicketAction(action, authToken, userId = "unknown") {
    ...
    if (action.deep_analysis) {
        try {
            const cfg = createResearchSubagentConfig(action.issue_key.split("-")[0], action.issue_key, userId);
...
export async function boardSweepAction(action, authToken) {
    const start = Date.now();
    const query = {
    ...
    if (action.cursor)
        query["cursor"] = action.cursor;
    if (action.base_url)
        query["base_url"] = action.base_url;
...
export async function confirmCommentAction(action, choice, authToken) {
    if (choice === "discard") {
...
export function getForgeAgentManifest() {
    return {
        key: "product-overlord-rovo-agent",
        name: "Product Overlord",
        description: "Analyses Jira tickets for readiness, maps affected components, and proposes " +
