...
import { analyseTicketAction, boardSweepAction, confirmCommentAction, getForgeAgentManifest, } from "../../forge/rovo-agent.js";
import { handleA2AEvent, buildA2AFallback, getAgentConnectorManifestEntry } from "../../forge/a2a-connector.js";
import { buildOperationalScope, assertScopeExcludes, createResearchSubagentConfig, assertSubagentIsolation, _resetRateLimits, _getRequestsToday, } from "../../forge/subagent.js";
...
describe("Forge agent end-to-end", () => {
    ...
    it("3.4  confirmCommentAction with choice=discard never calls confirm endpoint", async () => {
        const result = await confirmCommentAction({ run_id: "fake-run", csrf_token: "tok", approver_account_id: "user-1" }, "discard", AUTH);
    ...
    it("3.4  confirmCommentAction with choice=post goes through CSRF gate", async () => {
        ...
        const result = await confirmCommentAction({ run_id: runId, csrf_token: "wrong-token", approver_account_id: "user-1" }, "post", AUTH);
    ...
    it("3.3  load-more: second page has remaining issues", async () => {
        ...
        const second = await boardSweepAction({ board_id: 10, page_size: 2, cursor: first.next_cursor }, AUTH);
...
describe("A2A connector (EAP — tasks 4.x)", () => {
    it("4.2  returns feature_disabled:true when flag is false (default)", async () => {
        const response = await handleA2AEvent({ event_type: "assignment", issue_key: "PROJ-42", triggered_by_account_id: "user-1", timestamp: new Date().toISOString() }, "Bearer tok");
...
describe("Subagent scoping (tasks 5.x)", () => {
    ...
    it("5.3  rate limit enforced at 30 requests/user/day", () => {
        ...
        expect(() => createResearchSubagentConfig("PROJ", "PROJ-42", "rate-user")).toThrow(/rate limit exceeded/i);
