...
function makeIndex(componentNames) {
...
describe("mapTicketToComponents — high-confidence match", () => {
    it("returns auth component as top candidate when ticket mentions auth/oauth", () => {
        ...
        expect(result.candidate_components[0].confidence).toBeGreaterThan(result.candidate_components[1]?.confidence ?? 0);
    });
