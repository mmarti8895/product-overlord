...
// Helpers
// ---------------------------------------------------------------------------
function makeTicket(key = "PROJ-1", overrides = {}) {
    return {
        ticket_key: key,
        ticket_type: "story",
...
function makeIndex(componentName = "auth") {
    return {
...
describe("Hardening 4.3 — one parallel branch fails (repo)", () => {
    it("readiness continues when repo-mapping fails; failure reason in output", async () => {
        ...
        expect(result.actionPackage.evidence.some((e) => e.includes("unavailable") || e.includes("repo_map"))).toBe(true);
    });
});
// ---------------------------------------------------------------------------
// 4.4 — branch_name_suggestion always contains work-item key
