...
describe("RepoAdapter.getDecodedFileContent", () => {
    ...
    it("happy path: decodes base64 content and returns string", async () => {
        ...
        vi.stubGlobal("fetch", makeFetch([{ status: 200, body: { path: "src/auth.ts", content: encoded, encoding: "base64" } }]));
        const adapter = new RepoAdapter({ provider: "github", accessToken: "tok", retryDelayMs: 0 });
    ...
    it("throws FileTooLargeError when decoded content exceeds 100 KB", async () => {
        ...
        vi.stubGlobal("fetch", makeFetch([{ status: 200, body: { path: "big.ts", content: encoded, encoding: "base64" } }]));
        const adapter = new RepoAdapter({ provider: "github", accessToken: "tok", retryDelayMs: 0 });
        await expect(adapter.getDecodedFileContent("org", "repo", "big.ts", "main")).rejects.toThrow(FileTooLargeError);
    });
    it("retries on 5xx and eventually throws", async () => {
        ...
        const adapter = new RepoAdapter({ provider: "github", accessToken: "tok", retryDelayMs: 0 });
        await expect(adapter.getDecodedFileContent("org", "repo", "src/file.ts", "main")).rejects.toThrow();
    });
});
