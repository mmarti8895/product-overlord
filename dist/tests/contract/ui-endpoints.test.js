...
describe("GET /api/status", () => {
    ...
    it("5.5b — shadow_mode is true when featureFlags.shadowModeOnly=true", async () => {
        const shadowApp = createApp(makeConfig({ featureFlags: { repoGroundingEnabled: false, jiraIngestionEnabled: false, rovoMcpEnabled: false, shadowModeOnly: true, a2aEnabled: false, llmEnabled: false } }));
...
describe("forgeInstrumentation SSE broadcaster", () => {
    ...
    it("13.4a — LLM call events are fanned out to SSE subscribers", () => {
        ...
        expect(received[0].type).toBe("llm_call");
        unsub();
    });
});
// ---------------------------------------------------------------------------
