...
function makeStore() {
    return {
        ...
        getOKR: vi.fn().mockImplementation(async (id) => id === "okr-1" ? makeOKR() : null),
...
function makeApp(store) {
    const builder = { build: vi.fn().mockResolvedValue(makeSnapshot()) };
    const webhookMetrics = { push: vi.fn() };
    const app = new Hono();
