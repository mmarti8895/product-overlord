...
function makeSnapshot(withDigest = false) {
    return {
        portfolio_id: "p1",
...
function makeStore(snap) {
    return {
        ...
        getPortfolio: vi.fn().mockImplementation(async (id) => id === "p1" ? makePortfolio() : null),
...
function makeApp(store, snapForDigest = false) {
    const aggregator = { aggregate: vi.fn().mockResolvedValue(makeSnapshot()) };
    const digestWriter = {
