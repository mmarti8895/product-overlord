...
export function getEvalMetricsTrend() {
    ...
    const classificationAgreementTrend = caLast - caFirst > THRESHOLD ? "improving"
        : caFirst - caLast > THRESHOLD ? "degrading"
            : "stable";
    const precisionAt3Trend = p3Last - p3First > THRESHOLD ? "improving"
...
export function getPromotionQueueMetrics() {
...
export function checkLLMDegradedRateAlert(windowSize = 100) {
    ...
    if (events.length === 0)
        return { alert: false, degradedRate: 0 };
