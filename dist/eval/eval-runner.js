...
// Test helper
export function _clearEvalRuns() {
...
// ---------------------------------------------------------------------------
function buildRawIssue(entry) {
    const f = entry.fixture ?? { summary: entry.prompt };
    const fields = {
...
// ---------------------------------------------------------------------------
function computePrecisionAt3(expected, actual) {
    if (expected.length === 0)
        return 1; // vacuously correct
...
export async function runEvaluation(opts = {}) {
    ...
    const precisionAt3 = entriesWithComps.length > 0
    ...
    let classificationAgreementDelta;
    let precisionAt3Delta;
    if (prev) {
        ...
        if (classificationAgreementDelta < -REGRESSION_THRESHOLD ||
