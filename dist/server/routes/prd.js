...
export function createPRDRouter(store, writer, publisher) {
    ...
    router.get("/:projectKey/drafts/:id", async (c) => {
        ...
        if (!draft)
            return c.json(err("NOT_FOUND", "Draft not found"), 404);
    ...
    router.get("/:projectKey/drafts/:id/diff", async (c) => {
        ...
        if (!draft)
            return c.json(err("NOT_FOUND", "Draft not found"), 404);
        if (!draft.confluence_url)
            return c.json(ok({ before: "", after: "" }));
