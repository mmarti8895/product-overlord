...
export function createRoadmapRouter(store) {
    ...
    router.get("/:projectKey", c => {
        ...
        if (!snap)
            return c.json(err("NOT_FOUND", "No snapshot found. Try POST …/refresh"), 404);
    ...
    router.get("/:projectKey/epics/:epicKey", c => {
        ...
        if (!epic)
            return c.json(err("NOT_FOUND", "Epic not found"), 404);
