...
import { handleIngestIssue, handleIngestBoard, handleGetPlan, handleConfirmPost, } from "../forge/endpoints.js";
...
const ProviderSchemas = { jira: JiraSchema, openai: OpenAISchema, github: GitHubSchema };
// ---------------------------------------------------------------------------
...
export function createApp(config) {
    ...
    app.post("/kb/ingest", async (c) => {
        ...
        try {
            ...
            if (err instanceof StoreFullError)
                return c.json({ error: err.message }, 507);
    ...
    app.get("/kb/sources", async (c) => {
        ...
        if (!projectKey)
            return c.json({ error: "project_key query param required" }, 400);
        try {
    ...
    app.delete("/kb/sources/:id", async (c) => {
        ...
        if (!sourceId)
            return c.json({ error: "source id required" }, 400);
        try {
    ...
    if (config.uiDevEndpoints) {
        ...
        app.get("/api/config", (c) => {
            function mask(v) { return v ? "[set]" : "[not set]"; }
            return c.json({
    ...
    app.post("/api/agents/stop-all", (_c) => {
        ...
        for (const r of runs)
            AgentRegistry.stopRun(r.run_id);
    ...
    if (config.featureFlags.jiraIngestionEnabled && config.sprint?.boardIds?.length > 0) {
        ...
        app.route("/api/discovery", createDiscoveryRouter(triageQueue, themeClusterer, opportunitySizer, [webhookFeedback], webhookFeedback));
        ...
        app.get("/api/sprint/*", (c) => c.json({ ok: false, error: { code: "JIRA_NOT_CONFIGURED", message: "Jira integration is not enabled" } }, 503));
        app.get("/api/roadmap/*", (c) => c.json({ ok: false, error: { code: "JIRA_NOT_CONFIGURED", message: "Jira integration is not enabled" } }, 503));
    }
    // -------------------------------------------------------------------------
    // 404 fallback
    // -------------------------------------------------------------------------
