...
export class IngestionOrchestrator {
    ...
    async ingestBoard(boardId) {
        ...
        try {
            ...
            const { result, trace: rvTrace } = await this.rovo.searchIssues(`sprint in openSprints() ORDER BY created DESC`);
            traces.push(rvTrace);
            return {
    ...
    async ingestBacklog(boardId) {
        ...
        try {
            ...
            const { result, trace: rvTrace } = await this.rovo.searchIssues(`sprint is EMPTY AND statusCategory != Done ORDER BY created DESC`);
            traces.push(rvTrace);
            return {
                issues: result.issues,
                traces,
    ...
    async ingestSprint(sprintId) {
        ...
        try {
            ...
            const { result, trace: rvTrace } = await this.rovo.searchIssues(`sprint = ${sprintId} ORDER BY created DESC`);
            traces.push(rvTrace);
            return {
                issues: result.issues,
                traces,
        ...
        if (opts.accessibleProjects && opts.accessibleProjects.length > 0) {
            ...
            issues = issues.filter((i) => {
                ...
                if (!accessible.has(proj)) {
                    if (!inaccessibleProjects.includes(proj))
                        inaccessibleProjects.push(proj);
                    return false;
                }
                return true;
            });
