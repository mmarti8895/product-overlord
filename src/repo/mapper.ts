...
import type { ComponentIndex, ComponentDossier } from "./component-indexer.js";
...
function semanticScore(ticket: CanonicalTicket, dossier: ComponentDossier): number {
...
function structuralScore(ticket: CanonicalTicket, dossier: ComponentDossier): number {
  let score = 0;
...
function deriveCandidateTests(
  dossier: ComponentDossier
): { tests: CandidateTest[]; testLocationUnknown: boolean } {
  if (dossier.testDirs.length === 0) {
    return { tests: [], testLocationUnknown: true };
  }

  const tests: CandidateTest[] = dossier.testDirs.slice(0, 3).map((dir) => ({
    path: dir,
...
export function mapTicketToComponents(args: MapTicketArgs): RepoMapResult {
  ...
  const candidateComponents: CandidateComponent[] = top.map(({ dossier, sem, str, combined }) => {
    const why =
      `semantic=${sem.toFixed(2)} structural=${str.toFixed(2)} combined=${combined.toFixed(2)}`;
