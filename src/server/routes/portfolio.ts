...
export function createPortfolioRouter(
  ...
  digestWriter: PortfolioDigestWriter,
): Hono {
  ...
  // POST /api/portfolio
  router.post("/", async (c) => {
    const body = await c.req.json<{ name: string; project_keys: string[]; owner?: string }>();
  ...
  router.post("/:id/projects", async (c) => {
    const { project_key } = await c.req.json<{ project_key: string }>();
    try {
