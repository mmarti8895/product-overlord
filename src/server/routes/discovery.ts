...
export function createDiscoveryRouter(
  ...
  sizer: OpportunitySizer,
  adapters: FeedbackAdapter[],
  webhookAdapter: WebhookFeedbackAdapter,
): Hono {
  ...
  router.post("/ingest", async (c) => {
    const body = await c.req.json<{ source_id: string; text: string; created_at?: number; customer_segment?: string; tags?: string[] }>();
    webhookAdapter.push({
  ...
  router.post("/candidates/:id/promote", async (c) => {
    const body = await c.req.json<{ project_key: string; title: string; description: string }>();
    try {
