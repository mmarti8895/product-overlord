...
export function secretSave(key: string, value: string): void {
  ensureDir();
...
export function secretLoad(key: string): string | null {
  ensureDir();
  const filePath = join(STORE_DIR, `${encodeURIComponent(key)}.enc`);
  if (!existsSync(filePath)) return null;
  try {
    const masterKey = getMachineKey();
    const record: EncryptedRecord = JSON.parse(readFileSync(filePath, "utf8"));
...
export function secretDelete(key: string): void {
  const filePath = join(STORE_DIR, `${encodeURIComponent(key)}.enc`);
