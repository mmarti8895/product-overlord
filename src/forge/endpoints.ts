...
import type { ActionPackage, ReadinessStatus } from "../types/index.js";
...
const DEFAULT_BASE_URL =
  process.env["PLANNING_TOOL_BASE_URL"] ?? "https://product-overlord.internal";
...
function buildDeepLink(base: string, runId: string): string {
  return `${base}/runs/${runId}`;
}

function buildConfirmUrl(base: string, runId: string): string {
...
function buildSummary(pkg: ActionPackage): string {
...
function toEnvelope(
  ...
  const topMissing: ForgeEnvelope["top_missing_items"] = [
    ...pkg.manual_checks.slice(0, 3).map((c) => ({ dimension: c, severity: "medium" as const })),
    ...pkg.operational_risks.slice(0, 3).map((r) => ({ dimension: r, severity: "high" as const })),
  ].slice(0, 3);

  return {
    run_id: runId,
    summary: buildSummary(pkg),
    verdict: pkg.readiness_status as ReadinessStatus,
 ...
 */
function applySizeGuard<T extends { action_package?: unknown; status?: string; next_cursor?: string }>(
  response: T,
  runId: string
): T {
  if (byteLength(response) <= PAYLOAD_LIMIT_BYTES) return response;

  const truncated = { ...response } as T;
...
let _orchestrator: IngestionOrchestrator | null = null;

export function getOrchestrator(): IngestionOrchestrator {
  if (!_orchestrator) {
...
export function setOrchestrator(o: IngestionOrchestrator): void {
  _orchestrator = o;
}

// ---------------------------------------------------------------------------
// Simple HTTP-agnostic request context
// ---------------------------------------------------------------------------
...
function unauthorized(): ForgeResponse {
  return { status: 401, body: { error: "Unauthorized — Forge token required" } };
}

function isAuthorized(req: ForgeRequest): boolean {
...
export async function handleIngestIssue(
  req: ForgeRequest
): Promise<ForgeResponse<IngestIssueResponse>> {
  if (!isAuthorized(req)) return unauthorized() as ForgeResponse<IngestIssueResponse>;

  const body = req.body as Partial<IngestIssueRequest> | undefined;
  ...
  const result = await withTimeout(
    (async (): Promise<IngestIssueResponse> => {
      ...
      const response: IngestIssueResponse = {
        ...envelope,
  ...
  if (!result) {
    ...
    const envelope: IngestIssueResponse = {
      run_id: runId,
...
export async function handleIngestBoard(
  req: ForgeRequest
): Promise<ForgeResponse<IngestBoardResponse>> {
  if (!isAuthorized(req)) return unauthorized() as ForgeResponse<IngestBoardResponse>;
  ...
  const base = (req.query?.["base_url"] as string | undefined) ?? DEFAULT_BASE_URL;
  const pageSize = Math.min(Number(req.query?.["page_size"] ?? "25"), 50);
  const cursor = req.query?.["cursor"] as string | undefined;
  const start = Date.now();

  const result = await withTimeout(
    (async (): Promise<IngestBoardResponse> => {
      const orchestrator = getOrchestrator();
      ...
      const envelopes: BoardIssueEnvelope[] = [];
      ...
      const runId = randomUUID();
      const response: IngestBoardResponse = {
        run_id: runId,
        board_id: boardId,
        issues: envelopes,
...
export async function handleGetPlan(
  req: ForgeRequest
): Promise<ForgeResponse<GetPlanResponse>> {
  if (!isAuthorized(req)) return unauthorized() as ForgeResponse<GetPlanResponse>;
  ...
  const meta = (bundle as unknown as { metadata?: { action_package?: ActionPackage } }).metadata;
  const actionPackage: ActionPackage | undefined = meta?.action_package ?? undefined;

  const envelope: ForgeEnvelope | undefined = actionPackage
  ...
  return { status: 200, body: applySizeGuard(response as GetPlanResponse & { action_package?: unknown }, runId) as GetPlanResponse };
}
...
export async function handleConfirmPost(
  req: ForgeRequest
): Promise<ForgeResponse<ConfirmPostResponse>> {
  if (!isAuthorized(req)) return unauthorized() as ForgeResponse<ConfirmPostResponse>;
  ...
  const body = req.body as Partial<ConfirmPostRequest> | undefined;
  ...
  const readinessResult = (bundle as unknown as { scorer_output?: import("../types/index.js").ReadinessResult }).scorer_output;
  if (!readinessResult) {
