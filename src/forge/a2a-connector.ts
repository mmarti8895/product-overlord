...
const FEATURE_DISABLED_RESPONSE: A2AResponse = {
  accepted: false,
  feature_disabled: true,
  error:
    "rovo:agentConnector is feature-flagged off (FEATURE_ROVO_AGENT_CONNECTOR=false). " +
...
export async function handleA2AEvent(event: A2AEvent, authToken: string): Promise<A2AResponse> {
...
// ---------------------------------------------------------------------------
// 4.5  A2A unavailability fallback — returns graceful degradation info
// ---------------------------------------------------------------------------
...
export function buildA2AFallback(issueKey: string): A2AFallbackInfo {
...
// ---------------------------------------------------------------------------
// 4.3  Forge manifest registration helper (documentation / test helper)
//      In a real Forge app this would be in manifest.yml; here we expose a
//      runtime-readable config object used by integration tests.
// ---------------------------------------------------------------------------
...
export function getAgentConnectorManifestEntry(): ForgeManifestEntry {
  return {
    type: "rovo:agentConnector",
    key: "product-overlord-a2a",
    name: "Product Overlord A2A Connector",
    description:
      "EAP: projects the external orchestrator as an assignable Jira teammate via A2A. " +
