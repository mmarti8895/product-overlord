 ...
 */
export function assertScopeExcludes(scope: SubagentScope, otherProjectKey: string): boolean {
  ...
  const repoLeak = scope.repo_resources.some((r) =>
    r.toLowerCase().includes(otherProjectKey.toLowerCase())
  );
  const policyLeak = scope.policy_resources.some((r) =>
    r.toLowerCase().includes(otherProjectKey.toLowerCase())
  );
  const confluenceLeak =
    scope.confluence_space?.toUpperCase() === key;

  if (repoLeak || policyLeak || confluenceLeak) {
    throw new Error(
      `Scope violation: operational subagent for project ${scope.project_key} has access to project ${otherProjectKey}`
    );
  }
...
const researchRateLimitStore = new Map<string, number>();
 ...
 */
export function createResearchSubagentConfig(
  projectKey: string,
  issueKey: string,
  userId: string
  ...
  if (today >= RESEARCH_SUBAGENT_RATE_LIMIT) {
    throw new Error(
      `Research subagent rate limit exceeded for user ${userId} (${RESEARCH_SUBAGENT_RATE_LIMIT}/day)`
    );
  }
 ...
 */
export function assertSubagentIsolation(
  operationalSessionId: string,
  researchConfig: ResearchSubagentConfig
): void {
  if (operationalSessionId === researchConfig.session_id) {
    throw new Error(
      "Subagent isolation violation: research subagent shares session_id with operational subagent"
    );
  }
}
...
export function _getRequestsToday(userId: string): number {
