...
import type { DeepResearchFinding } from "../../forge/deep-research.js";
...
function makeJobStub(delayMs = 0): (sessionId: string) => Promise<{
  findings: DeepResearchFinding[];
  enriched_readiness: Record<string, unknown>;
}> {
...
describe("5.4b – timeout handling", () => {
  ...
  it("timeout result is logged to evidence store", async () => {
    ...
    const bundle = evidenceStore.get(result.evidence_run_id!) as unknown as Record<string, unknown>;
  ...
  it("all returned findings have source: 'deep-research'", async () => {
    const result = await runDeepResearch({
      ...
      analysisJob: async (_sid) => ({
        findings: [
          { source: "deep-research" as const, text: "Finding A", confidence: 0.8 },
          { source: "deep-research" as const, text: "Finding B", confidence: 0.6, dimension: "scope_boundaries" },
        ],
        enriched_readiness: {},
      }),
      timeoutMs: 500,
