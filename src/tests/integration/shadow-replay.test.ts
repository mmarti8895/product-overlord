...
function makeIssue(key: string, type = "story", hasCriteria = true): RawIssue {
  return {
    key,
    fields: {
...
describe("3.4d – evidence drill-down", () => {
  it("drillDownEvidence returns an evidence bundle for a valid audit record", async () => {
    ...
    expect((bundle as { run_id?: string }).run_id).toBe(record.evidence_run_id);
  });
