...
describe("4.3 profile_source: default logged when no project profile exists", () => {
  it("scorer logs profile_source=default for unknown project", () => {
    ...
    const defaultLogCall = logSpy.mock.calls.find(
      (args) =>
        args[0] === "readiness_score" &&
        (args[1] as Record<string, unknown>)?.profile_source === "default"
    );
...
describe("4.4 security — credential hygiene and evidence isolation", () => {
  ...
  it("evidence store entries are isolated per run_id — no cross-run data leak", () => {
    ...
    const makeBundle = (key: string) => ({
      trigger: `direct_key:${key}`,
      adapter_traces: [],
      canonical_ticket: { ticket_key: key } as unknown as CanonicalTicket,
      scorer_input: { profile_id: "default:story", profile_source: "default" as const },
      scorer_output: {
        ticket_key: key,
        ticket_type: "story",
        ...
        explanation: "",
        evidence: [],
      } as import("../../types/index.js").ReadinessResult,
