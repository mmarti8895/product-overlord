...
import type { ActionPackage } from "../../types/index.js";

// ---------------------------------------------------------------------------
// Fixture factory
// ---------------------------------------------------------------------------

function makePackage(overrides: Partial<ActionPackage> = {}): ActionPackage {
  return {
    ticket_key: "ABC-123",
...
describe("Reviewer — insufficient evidence", () => {
  it("blocks when readiness_score < minScore and no conflict explanation", () => {
    const verdict = reviewActionPackage(
      makePackage({ readiness_score: 30, conflict: null }),
...
describe("Reviewer — branch key", () => {
  it("blocks when branch_name_suggestion does not contain ticket key", () => {
    const verdict = reviewActionPackage(
      makePackage({ branch_name_suggestion: "feature-add-oauth" }) // missing 'abc-123'
    );
    expect(verdict.approved).toBe(false);
...
describe("Reviewer — slug validation", () => {
  it("blocks when openspec_change_slug contains uppercase", () => {
    const verdict = reviewActionPackage(
      makePackage({ openspec_change_slug: "ABC-123-oauth" })
    );
    expect(verdict.approved).toBe(false);
