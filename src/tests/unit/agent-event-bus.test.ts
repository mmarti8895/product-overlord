...
function makeStart(agent: string, run_id = "run-1"): AgentEvent {
  return { event: "start", agent, run_id, ts: new Date().toISOString() };
}

function makeProgress(agent: string, run_id = "run-1", pct = 50): AgentEvent {
  return { event: "progress", agent, run_id, pct, msg: "working", ts: new Date().toISOString() };
}

function makeFinish(agent: string, run_id = "run-1"): AgentEvent {
...
describe("AgentEventBus", () => {
  ...
  describe("helper emitters", () => {
    ...
    it("progress() emits a progress event with pct", () => {
      ...
      if (e?.event === "progress") expect(e.pct).toBe(42);
    });
