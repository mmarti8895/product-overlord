...
import type { JiraConfig, OpenAIConfig, GitHubConfig } from "../../connections/ConnectionManager.js";

// ─── helpers ────────────────────────────────────────────────────────────────

function makeJira(overrides: Partial<JiraConfig> = {}): JiraConfig {
  return { baseUrl: "https://example.atlassian.net", projectKey: "PROJ", token: "secret-token", ...overrides };
}

function makeOpenAI(overrides: Partial<OpenAIConfig> = {}): OpenAIConfig {
  return {
    apiKey: "sk-secret",
    plannerModel: "gpt-4o",
...
function makeGitHub(overrides: Partial<GitHubConfig> = {}): GitHubConfig {
...
describe("ConnectionManager", () => {
  ...
  describe("save", () => {
    it("calls secretSave with serialised jira config", async () => {
      ...
      expect(SecretStore.secretSave).toHaveBeenCalledWith(
        "overlord.connection.jira",
        JSON.stringify(cfg),
      );
    });
  ...
  describe("load (masked)", () => {
    ...
    it("masks token field for jira", async () => {
      ...
      expect((result as JiraConfig).token).toBe("***");
      expect((result as JiraConfig).baseUrl).toBe(cfg.baseUrl);
      expect((result as JiraConfig).projectKey).toBe(cfg.projectKey);
    });
    ...
    it("masks apiKey field for openai", async () => {
      ...
      expect((result as OpenAIConfig).apiKey).toBe("***");
      expect((result as OpenAIConfig).plannerModel).toBe(cfg.plannerModel);
    });

    it("masks pat and privateKey for github", async () => {
      const cfg: GitHubConfig = { ...makeGitHub(), privateKey: "pem-secret" };
      vi.mocked(SecretStore.secretLoad).mockResolvedValueOnce(JSON.stringify(cfg));
      const result = await ConnectionManager.load("github");
      expect((result as GitHubConfig).pat).toBe("***");
      expect((result as GitHubConfig).privateKey).toBe("***");
      expect((result as GitHubConfig).repos).toEqual(cfg.repos);
    });
  });
  ...
  describe("loadRaw (unmasked)", () => {
    it("returns original config without masking", async () => {
      ...
      expect((result as JiraConfig).token).toBe("secret-token");
    });
