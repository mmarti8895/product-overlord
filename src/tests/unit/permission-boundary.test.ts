...
function buildRawIssue(key: string, summary: string): RawIssue {
  return {
    key,
    fields: {
      summary,
...
describe("4.2 – zero cross-boundary data in evidence store", () => {
  it("two projects get separate evidence run_ids", () => {
    ...
    const e1 = evidenceStore.get(b1) as unknown as { ticket_key: string };
    const e2 = evidenceStore.get(b2) as unknown as { ticket_key: string };
...
describe("4.2 – zero cross-boundary data in shadow replay audit records", () => {
  it("replay for project ALPHA produces records only with ALPHA keys", async () => {
    ...
    const records = getAuditRecords().filter(
      (r) => r.replay_run_id === report.replay_run_id
    );
...
describe("4.4 – evidence store access controls", () => {
  it("evidence store returns a copy, not a mutable reference", () => {
    ...
    const retrieved = evidenceStore.get(runId) as unknown as Record<string, unknown>;

    // Mutating the retrieved object must not affect the stored copy
    if (retrieved) {
      (retrieved as Record<string, unknown>)["data"] = "tampered";
    }

    const retrieved2 = evidenceStore.get(runId) as unknown as Record<string, unknown>;
...
describe("4.4 – research subagent isolation", () => {
  ...
  it("shared session_id throws isolation violation", () => {
    ...
    expect(() => assertSubagentIsolation(config.session_id, config)).toThrow(
      "isolation violation"
    );
  });
});
