...
import type { RawTreeEntry } from "../../repo/repo-adapter.js";

// ---------------------------------------------------------------------------
// Fixtures
// ---------------------------------------------------------------------------

function makeEntries(paths: string[]): RawTreeEntry[] {
  return paths.map((p) => ({ path: p, type: "blob" as const }));
}

const TYPICAL_ENTRIES: RawTreeEntry[] = makeEntries([
  "src/auth/index.ts",
  "src/auth/login.ts",
...
describe("ComponentIndexer — diff()", () => {
  ...
  it("marks components as updated when root paths change", () => {
    ...
    const { updated, unchanged } = indexer.diff(
      prev,
      makeEntries(["src/foo.ts", "src/bar.ts"])
    );
  ...
  it("marks a brand-new component as updated", () => {
    const prev = indexer.index("org/repo", makeEntries(["src/foo.ts"]));
    const { updated } = indexer.diff(
      prev,
      makeEntries(["src/foo.ts", "infra/terraform.tf"])
    );
...
describe("RepoAdapter — > 20 GB rejection guard", () => {
  it("throws when repo size exceeds 20 GB", async () => {
    ...
    await expect(adapter.getRepoMeta("org", "big-repo")).rejects.toThrow(
      /exceeds.*20 GB/i
    );
  });
});
