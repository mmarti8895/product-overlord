...
function makeIssue(sp: number, status: string) {
...
describe("VelocityTracker", () => {
  describe("Fixture A — 6 closed sprints", () => {
    ...
    const sprintDefs = Array.from({ length: 6 }, (_, i) => ({ id: 6 - i, name: `Sprint ${6 - i}`, state: "closed" as const }));

    beforeEach(() => {
      const jira = {
        listSprints: vi.fn().mockResolvedValue({ sprints: sprintDefs, trace: {} }),
        getSprintIssues: vi.fn().mockImplementation(async (id: number) => ({
    ...
    it("committed >= completed for all points", async () => {
      const pts = await tracker.getVelocity(1, 6);
      for (const p of pts) expect(p.committed).toBeGreaterThanOrEqual(p.completed);
    });
  });
