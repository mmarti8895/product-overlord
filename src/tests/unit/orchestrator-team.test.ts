...
function emitStart(agent: string, run_id: string) {
  AgentEventBus.emit({ event: "start", agent, run_id, ts: new Date().toISOString() });
}

function emitProgress(agent: string, run_id: string) {
...
describe("OrchestratorTeam", () => {
  ...
  describe("thrash detection", () => {
    it("emits a critical finding when agent exceeds event threshold", () => {
      ...
      const thrashFinding = findings.find(
        f => f.agent === agent && f.type === "thrash" && f.severity === "critical",
      );
    ...
    it("finding status starts as open", () => {
      ...
      for (let i = 0; i < 12; i++) emitStart(agent, run_id);
