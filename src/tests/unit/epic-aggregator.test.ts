...
function makeJira(epics: { key: string; summary: string; fix_versions?: string[] }[]): JiraAgileRestAdapter {
  return {
    getEpicsForBoard: vi.fn().mockResolvedValue({ epics }),
  } as unknown as JiraAgileRestAdapter;
}

function makeTicket(overrides: Partial<CanonicalTicket> & { readiness_score?: number } = {}): CanonicalTicket {
  return {
    key: "PROJ-1",
    summary: "A ticket",
    status: "To Do",
  ...
  } as unknown as CanonicalTicket;
}

function makeAggregator(
  epics: { key: string; summary: string; fix_versions?: string[] }[],
  childMap: Record<string, (CanonicalTicket & { readiness_score?: number })[]>,
): EpicAggregator {
  const jira = makeJira(epics);
  return new EpicAggregator({
    jira,
    loadChildTickets: vi.fn().mockImplementation(async (epicKey: string) => childMap[epicKey] ?? []),
  });
}
...
describe("EpicAggregator", () => {
  ...
  it("healthy board — high readiness children produce healthy label", async () => {
    const agg = makeAggregator(
      [{ key: "PROJ-100", summary: "Epic A" }],
      {
        "PROJ-100": [
          makeTicket({ readiness_score: 0.9 } as never),
          makeTicket({ readiness_score: 0.85 } as never),
        ],
      },
    );
  ...
  it("at-risk board — mid readiness children produce at-risk label", async () => {
    const agg = makeAggregator(
      [{ key: "PROJ-200", summary: "Epic B" }],
      {
        "PROJ-200": [
          makeTicket({ readiness_score: 0.55 } as never),
          makeTicket({ readiness_score: 0.45 } as never),
        ],
      },
    );
    const snap = await agg.aggregate(1, "PROJ");
  ...
  it("health_score 70 → healthy", async () => {
    const agg = makeAggregator(
      [{ key: "PROJ-400", summary: "Epic D" }],
      { "PROJ-400": [makeTicket({ readiness_score: 0.70 } as never)] },
    );
    const snap = await agg.aggregate(1, "PROJ");
    expect(snap.epics[0].health_label).toBe("healthy");
  });
  ...
  it("health_score 40 → at-risk", async () => {
    const agg = makeAggregator(
      [{ key: "PROJ-500", summary: "Epic E" }],
      { "PROJ-500": [makeTicket({ readiness_score: 0.40 } as never)] },
    );
    const snap = await agg.aggregate(1, "PROJ");
    expect(snap.epics[0].health_label).toBe("at-risk");
  });
  ...
  it("health_score < 40 → blocked", async () => {
    const agg = makeAggregator(
      [{ key: "PROJ-600", summary: "Epic F" }],
      { "PROJ-600": [makeTicket({ readiness_score: 0.20 } as never)] },
    );
    const snap = await agg.aggregate(1, "PROJ");
  ...
  it("maps child ticket fix_versions to milestones", async () => {
    ...
    const agg = makeAggregator(
      [{ key: "PROJ-700", summary: "Epic G" }],
      { "PROJ-700": [childWithVersion as unknown as CanonicalTicket & { readiness_score?: number }] },
    );
    const snap = await agg.aggregate(1, "PROJ");
