...
function mockOrchestrator(issues: ReturnType<typeof rawIssue>[]) {
  ...
  return mock as unknown as IngestionOrchestrator;
}
...
describe("GET /forge/ingest/board/{id} — happy path", () => {
  it("returns board envelope with issues array", async () => {
    ...
    const res = await handleIngestBoard(
      makeReq(AUTH_HEADER, undefined, { id: "42" }, { id: "42", page_size: "2" })
    );
    expect(res.status).toBe(200);
  ...
  it("load-more: second page uses cursor from first page", async () => {
    setOrchestrator(mockOrchestrator([rawIssue("ABC-1"), rawIssue("ABC-2"), rawIssue("ABC-3")]));
    const first = await handleIngestBoard(
      makeReq(AUTH_HEADER, undefined, { id: "42" }, { id: "42", page_size: "2" })
    );
    const cursor = first.body.next_cursor!;
    const second = await handleIngestBoard(
      makeReq(AUTH_HEADER, undefined, { id: "42" }, { id: "42", page_size: "2", cursor })
    );
  ...
  it("each issue envelope has issue_index", async () => {
    ...
    const res = await handleIngestBoard(
      makeReq(AUTH_HEADER, undefined, { id: "42" }, { id: "42", page_size: "25" })
    );
...
describe("POST /forge/output/confirm/{run_id} — CSRF / auth", () => {
  it("returns 403 when CSRF token is invalid", async () => {
    const res = await handleConfirmPost(
      makeReq(
        AUTH_HEADER,
        { csrf_token: "wrong-token", approver_account_id: "user-1" },
        { run_id: "fake-run" }
      )
    );
  ...
  it("returns 400 when csrf_token or approver_account_id is missing", async () => {
    const res = await handleConfirmPost(
      makeReq(AUTH_HEADER, { csrf_token: "" }, { run_id: "some-run" })
    );
    expect(res.status).toBe(400);
  });
  ...
  it("returns 404 for unknown run_id even with correct header", async () => {
    ...
    const res = await handleConfirmPost(
      makeReq(
        AUTH_HEADER,
        { csrf_token: "good-token", approver_account_id: "user-1" },
        { run_id: "valid-run" }
      )
    );
