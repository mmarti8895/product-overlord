...
import type { Epic, Milestone, RoadmapSnapshot } from "../types/roadmap.js";
...
function deriveHealthLabel(score: number): Epic["health_label"] {
...
export class EpicAggregator {
  constructor(private readonly deps: EpicAggregatorDeps) {}

  async aggregate(boardId: number, projectKey: string): Promise<Omit<RoadmapSnapshot, "dependency_graph">> {
    ...
    for (const raw of rawEpics) {
      ...
      if (children.length > 0) {
        const mean = children.reduce((s, c) => s + ((c as unknown as { readiness_score?: number }).readiness_score ?? 0), 0) / children.length;
...
function _quarterFromDate(isoDate: string): string {
