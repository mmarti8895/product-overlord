...
export class ConfluencePublisher {
  ...
  async publish(draft: PRDDraft): Promise<{ url: string; diff: PRDDiff }> {
    ...
    if (searchResp.ok) {
      const data = (await searchResp.json()) as { results: { id: string; version: { number: number }; body: { storage: { value: string } } }[] };
    ...
    if (existingId) {
      ...
      const data = (await resp.json()) as { _links: { webui: string } };
