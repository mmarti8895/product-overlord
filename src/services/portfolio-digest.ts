...
export class PortfolioDigestWriter {
  constructor(
    ...
    private readonly cfg: DigestConfig,
  ) {}

  async generate(snapshot: PortfolioSnapshot): Promise<PortfolioDigest> {
    ...
    const prompt =
      `You are a portfolio manager. Write a concise Markdown portfolio digest (≤4 paragraphs) covering:\n` +
      `- Overall health\n- Key risks\n- Cross-team dependencies (count: ${depCount})\n- Recommended actions\n\n` +
      `Projects:\n${projectLines}`;

    let markdown = `# Portfolio Digest\n\nGenerated at ${snapshot.generated_at}\n\n${projectLines}`;
    try {
      const { result } = await this.llm.complete<{ markdown: string }>(prompt, DIGEST_SCHEMA);
  ...
  async deliverToSlack(digest: PortfolioDigest): Promise<DeliveryRecord> {
  ...
  async deliverToConfluence(digest: PortfolioDigest): Promise<DeliveryRecord> {
    const record: DeliveryRecord = {
      id:           randomUUID(),
      portfolio_id: digest.portfolio_id,
      channel:      "confluence",
