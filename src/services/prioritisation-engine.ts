...
import type { Epic, RICEScore, ICEScore } from "../types/roadmap.js";
import type { LLMAdapter } from "../llm/types.js";
import { logger } from "../utils/logger.js";

const IMPACT_LABEL_MAP: Record<string, number> = {
  critical: 3,
  high: 2,
  medium: 1,
  low: 0.5,
...
export class PrioritisationEngine {
  ...
  async score(epic: Epic, overrides?: Partial<RICEScore>): Promise<Epic> {
    let rice: RICEScore | null = null;
    let ice: ICEScore | null = null;
  ...
  private async _computeRICE(epic: Epic, overrides?: Partial<RICEScore>): Promise<RICEScore> {
    // --- Reach: LLM estimate ---
    ...
    if (overrides?.reach === undefined) {
      try {
        ...
        const { result } = await this.llm.complete<{ reach: number }>(promptStr, schema);
  ...
  private _computeICE(rice: RICEScore): ICEScore {
